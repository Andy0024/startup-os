...
export class CodeChangesComponent implements OnInit,
  ...
  initLines(): void {
    ...
    this.changesService.applyTextDiffBlockLines(leftBlockLines, rightBlockLines,
                                                this.textDiff, );
    ...
    for (const section of this.sections) {
      ...
      } = this.blocksService.synchronizeBlockLines(leftBlockLines,
                                                   rightBlockLines,
  ...
  startThread(thread: Thread): void {
    ...
    this.commentsService.addThread(changesLines[lineIndex], blockIndex,
                                   thread, );

    // Save thread to threads map for fast access
    this.commentsService.saveAsOpen(thread.getLineNumber(), lineIndex,
                                    blockIndex, this.openThreadsMap, );
  }

  // Creates new thread and sends it to firebase
  addThread(value: string, blockLine: BlockLine, blockIndex: number,
    ...
    this.commentsService.saveAsOpen(blockLine.lineNumber, lineIndex, blockIndex,
                                    this.openThreadsMap, );
  }

  // Send diff with new comment to firebase
  private addComment(lineNumber: number, comment: Comment, thread: Thread,
