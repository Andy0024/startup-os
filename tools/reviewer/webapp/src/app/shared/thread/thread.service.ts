...
export class ThreadService {
  ...
  getThread(diff: Diff, thread: Thread): Thread {
    const diffThread: Thread =
        this.getThreadFromList(diff.getDiffThreadList(), thread);
    if (diffThread) {
      return diffThread;
    }
    const codeThread: Thread =
        this.getThreadFromList(diff.getCodeThreadList(), thread);
    if (codeThread) {
      return codeThread;
    }
  }
  ...
  private getThreadFromList(threadList: Thread[],
                            targetThread: Thread): Thread {
