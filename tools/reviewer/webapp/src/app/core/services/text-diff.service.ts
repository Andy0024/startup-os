...
export class TextDiffService {
  ...
  load(diff: Diff, filenameWithRepo: string, defaultLeftCommitId?: string,
       defaultRightCommitId?: string, ): Observable<TextDiffReturn> {
          ...
          .subscribe(branchInfoList => {
            ...
            try {
              const {branchInfo, file} = this.localserverService.getFileData(
                  filenameWithRepo, branchInfoList, );

              // Create commits
              const {
                  leftFile, rightFile, filesSortedByCommits,
              } = this.getFiles(file, branchInfo, defaultLeftCommitId,
                                defaultRightCommitId, );

              // Create local threads
              const localThread: Thread[] =
                  this.getLocalThreads(diff, file, leftFile.getCommitId(),
                                       rightFile.getCommitId(), );

              // Load textDiff
              this.localserverService.getFileChanges(leftFile, rightFile)
  ...
  private getFiles(file: File, branchInfo: BranchInfo, leftCommitId: string,
                   rightCommitId: string, ):
      {leftFile : File; rightFile : File; filesSortedByCommits : File[];} {
    const filesSortedByCommits: File[] =
        this.localserverService.getFilesSortedByCommits(file, branchInfo, );
    ...
    let leftIndex: number =
        this.addCommitId(leftCommitId, filesSortedByCommits);
    let rightIndex: number =
        this.addCommitId(rightCommitId, filesSortedByCommits);
  ...
  private getLocalThreads(diff: Diff, file: File, leftCommitId: string,
                          rightCommitId: string, ): Thread[] {
    return diff.getCodeThreadList().filter(thread => {
      const filenamesAreEqual: boolean =
          thread.getFile().getFilenameWithRepo() === file.getFilenameWithRepo();
