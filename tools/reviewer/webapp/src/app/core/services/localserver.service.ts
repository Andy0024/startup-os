...
const CLOUD_FUNCTIONS: string =
    'https://us-central1-startupos-5f279.cloudfunctions.net';
...
export class LocalserverService {
  ...
  constructor(
      private http: HttpClient, private encodingService: EncodingService,
  ...
  getBranchInfoList(id: number, workspace: string): Observable<BranchInfo[]> {
    return new Observable(observer => {
      ...
      const requestBase64: string =
          this.encodingService.encodeUint8ArrayToBase64String(requestBinary);

      // Send the request to local server
      this.http
          .get(
              this.server + '/get_diff_files?request=' + requestBase64,
              {responseType: 'text'})
          .subscribe(
              diffFilesResponseBase64 => {
                // Decode response
                const diffFilesResponseBinary: Uint8Array =
                    this.encodingService.decodeBase64StringToUint8Array(
                        diffFilesResponseBase64);
                const diffFilesResponse: DiffFilesResponse =
                    DiffFilesResponse.deserializeBinary(
                        diffFilesResponseBinary);
              ...
              () => {
                this.fallback().subscribe(() => {
                  this.getBranchInfoList(id, workspace)
                      .subscribe(branchInfo => observer.next(branchInfo), );
                }, () => observer.error());
              });
    });
  }

  getFileChanges(leftFile: File, rightFile: File, ):
    ...
    return new Observable(observer => {
      ...
      this.http
          .get(
              this.server + '/get_text_diff?request=' + requestBase64,
              {responseType: 'text'})
          .subscribe(
              textDiffResponseBase64 => {
                // Decode response
                const textDiffResponseBinary: Uint8Array =
                    this.encodingService.decodeBase64StringToUint8Array(
                        textDiffResponseBase64);
                const textDiffResponse: TextDiffResponse =
                    TextDiffResponse.deserializeBinary(textDiffResponseBinary);

                observer.next(textDiffResponse);
              },
              () => {
                this.fallback().subscribe(() => {
                  this.getFileChanges(leftFile, rightFile)
                      .subscribe(
                          textDiffResponse =>
                              observer.next(textDiffResponse), );
                }, () => observer.error());
  ...
  private addWithReplace(
      fileDictionary: {[filename: string]: File}, newfiles: File[], ): void {
  ...
  // Then we get those files: Head File, F1, F2, F3, F4, ..., FN, Uncommitted
  // File
  // Basically all the files have same filename, difference is just in commit id
  // and action.
  getFilesSortedByCommits(file: File, branchInfo: BranchInfo): File[] {
    ...
    const headFile =
        File.deserializeBinary(file.serializeBinary());  // Copy proto object
    ...
    for (const commit of branchInfo.getCommitList()) {
      for (const committedFile of commit.getFileList()) {
        if (committedFile.getFilenameWithRepo() ===
    ...
    for (const uncommittedFile of branchInfo.getUncommittedFileList()) {
      if (uncommittedFile.getFilenameWithRepo() ===
          file.getFilenameWithRepo()) {
